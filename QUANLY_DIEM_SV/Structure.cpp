...
int ControlSinhVienDkyLTC(AppContext& context, int positionSubmenu) {
	return Search_SV_Dky_LTCByConditions(context, positionSubmenu);
}
// ===== END HELPER =====
...
bool Kiem_Tra_Trung_MH_NK_HK(Lop_Tin_Chi* ltc[], int n, Lop_Tin_Chi* selected) {
	for (int i = 0; i < n; i++)
	{
		if (_strcmpi(ltc[i]->MAMH, selected->MAMH) == 0
			&& _strcmpi(ltc[i]->NIEN_KHOA, selected->NIEN_KHOA) == 0
			&& ltc[i]->HOC_KY == selected->HOC_KY) {
			return true;
		}
	}
	return false;
			...
			if (!CheckInputBoxIsNull(Texts, 4)) {
				...
				if (single != NULL) {
					...
					if (ds_sv_dky.totalSv != 0 && !isScoreFeature) {
						key = CommonShowSvList(context, ds_sv_dky, positionSubmenu, NULL);
						if (key == key_esc) {
							/*delete singleCloned;
							delete[]context.ds;*/
							break;
			...
			if (!CheckInputBoxIsNull(Texts, 2)) {
				...
				if (arrayLTC != NULL) {
					...
					do
					{
						...
						key = InDanhSachLopTinChi(context, arrayLTC, total, 40, 6, positionSubmenu, true, true);
...
bool CheckNienKhoa_InValid(string nienkhoa) {
	...
	return (first < 2000 || last > 2050 || first >= last);
}
// ===== END DS LOP TIN CHI =====
...
bool CheckMonHocExistLTC(Lop_Tin_Chi* ds_ltc[], int nLTC, char* mamh) {
	...
	{
		if (_strcmpi(ds_ltc[i]->MAMH, mamh) == 0) {
			return true;
		}
	}
	return false;
...
void ShowSingleSV(SINH_VIEN* sv, int index) {
	cout << setw(3) << index + 1 << char(179) << setw(15) << sv->MALOP << char(179)
		...
		<< setw(5) << sv->PHAI << char(179) << setw(11) << sv->SDT << char(179) << setw(12) << sv->NAMNHAPHOC << char(179);
}
...
int CommonShowSvList(AppContext context, DS_SINH_VIEN& ds_sv, int positionSubMenu, char* maLop = NULL) {
	...
	return InDanhSachSinhVien(context, ds_sv, sv, total, 35, 10, positionSubMenu);
}
	...
	do {
		...
		case key_F2: {
			...
			if (key == key_Enter && ActionConfirm.posStatus == 0) {
				Lop_Tin_Chi** dsLTC = CreateArrayLopTinChi(context.nLTC, sizeof(Lop_Tin_Chi));
				int nLTC_Tmp = 0;
				ConvertTreeToArray(context.tree, dsLTC, nLTC_Tmp);
				bool exist = CheckMonHocExistLTC(dsLTC, nLTC_Tmp, ds_mh.ds[posActive + posPrint]->MAMH);
				if (!exist) {
					...
					gotoXY(50, 25);
					cout << "Xoa that bai. Mon hoc dang duoc mo LTC";
					Sleep(1500);
				}
			...
			if (isStudent) {
				...
				if (isInsert && ltc[posActive + posPrint]) {
					if (ltc[posActive + posPrint]->sv_max == ltc[posActive + posPrint]->totalSvDK) {
						gotoXY(55, 39);
						string a = "Lop tin chi da full. Khong the dang ky!";
						cout << a;
						ClearMessage(55, 39, a.length());
					}
					else {
						...
						if (!exist) {
							int totalSv_Dky = 0;
							Lop_Tin_Chi** ltc_sv_dky = TimLopTinChiSinhVienDaDangKy(ltc, context.nLTC, totalSv_Dky, context.currentUser);
							bool result = Kiem_Tra_Trung_MH_NK_HK(ltc_sv_dky, totalSv_Dky, ltc[posActive + posPrint]);
							if (result) {
								gotoXY(55, 39);
								cout << "Lop hoc da duoc dang ky voi so nhom khac!";
								Sleep(1000);
								clrscr(55, 39, 90, 2, ' ');
								delete[]ltc_sv_dky;
								break;
				...
				if (keyFormUpdate == key_Enter) {
					if (!CheckInputBoxIsNull(textFields, 7)) {
						...
						if (!exist) {
							...
							else if (CheckNienKhoa_InValid(textFields[2])) {
								gotoXY(50, 28);
								SetColor(color_red);
								cout << "Nien khoa khong hop le (>= 2000 & <=2050). Ex: 2015-2020";
								Sleep(1500);
								valid = false;
							}
							else {
...
int InDanhSachSinhVien(AppContext context, DS_SINH_VIEN& ctx_ds_sv, SINH_VIEN* ds_sv[], int n, int x, int y, int positionSubMenu) {
	...
	cout << setw(3) << "STT" << char(179) << setw(15) << "Ma Lop" << char(179) << setw(13) << "Ma SV" << char(179)
		...
		<< setw(5) << "Phai" << char(179) << setw(11) << "SDT" << char(179) << setw(12) << "Nam nhap hoc" << char(179);
	...
	do {
		...
		case key_F2: {
			...
			key = ControlMenu(&ActionConfirm, color_darkwhite, color_green);
			if (key == key_Enter && ActionConfirm.posStatus == 0) {
				// TODO: Check student exist in ltc => do not remove
				Lop_Tin_Chi** ds_ltc = CreateArrayLopTinChi(context.nLTC, sizeof(Lop_Tin_Chi));
				int nLtc_tmp = 0;
				ConvertTreeToArray(context.tree, ds_ltc, nLtc_tmp);
				bool exist = CheckSinhVienExistLTC(ds_ltc, nLtc_tmp, ds_sv[posActive + posPrint]->MASV);
				if (!exist) {
				...
				clrscr(35, 4, 105, 35, ' '); // xoa tu vi tri form confirm remove
				return -1;
			}
			else {
				clrscr(35, 4, 105, 35, ' '); // xoa tu vi tri form confirm remove
		...
		case key_F3: {
			...
			do
			{
				...
				rectagle(40, 5, 65, 25);
				SetColor(color_white);
				keyFormUpdate = DrawFormInputSinhVien(65, 6, 30, textFields, maxTexts, 7, true);
